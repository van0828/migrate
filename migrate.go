...
func (m *Migrate) Up() error {
	...
	fmt.Println(fmt.Sprintf("\nTry to do up. curVersion:%+v dirty:%+v", curVersion, dirty))
...
func (m *Migrate) readUp(from int, limit int, ret chan<- interface{}) {
	...
	fmt.Println(fmt.Sprintf("\nTry to read up. from:%+v limit:%+v", from, limit))
	// check if from version exists
	if from >= 0 {
		if err := m.versionExists(suint(from)); err != nil {
...
func (m *Migrate) runMigrations(ret <-chan interface{}) error {
	...
	for r := range ret {
		fmt.Println(fmt.Sprintf("Try to loop run migrations."))
		if m.stop() {
		...
		case *Migration:
			...
			if migr.Body != nil {
				fmt.Println(fmt.Sprintf("Try to run migrations. LogString:%+v", migr.LogString()))
